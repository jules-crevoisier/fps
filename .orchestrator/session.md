...
- Cleanup agent done: boot straight into Wasteland TDM (scenes/boot.tscn, QuickStart.gd), Ravage only, Verrou only, no abilities, UI = crosshair+hitmarker, TDM only; tests 2032/2035. Committed+pushed 728d3c9 (-29.6k lines).
  Left (do in map phase): MapSetup.gd still news HardpointMode/SnD/Duel (files kept; delete after MapSetup edit); PauseMenu/BuyMenu stubs attached in map scenes; tools/review/gameplay_probe.gd + ui_shots.gd reference deleted classes (CI probe step will fail) -> fix/delete; assets sweep (delete unreferenced models/incoming/textures after removing art modules); other map scenes to delete.
  WAITING: user approval of plan v6.
- USER on v6: 'symétrique, boring à la mort' -> ONE Opus designer on v7 asymmetric-but-balanced (W town vs E different identity; bent street, irregular plaza, diagonal canyon, different heights; balance checks ±10 % contact times, equal power positions, sightlines ±15 %, area ±10 %). Show before generating.
- v7 asymmetric done (W town / E freight yard + mine; no mirror; 23 checks pass; contact per lane W/E 5.33/5.71, 5.56/5.65, 7.54/7.55 s; 3 power positions each; area 2464/2575 m²). New JSON keys (mirror.enabled, platform, levels.dock, routes) must be supported by the game generator. Sent to user, WAITING approval.
- USER approved v7 ('fais la carte block que je puisse la tester'). Committed plan v7 locally. ONE sonnet builder: wasteland.gd from JSON + dev grid shader + bots from JSON + tests + windowed screenshots (reports/checkpoints/2026-09-26_greybox-v7/). Then separate cleanup (other scenes, mode files, art, assets, tools) and push.
- 92a9a6a pushed: v7 greybox generated from JSON (wasteland.gd, bots from plan, dev_grid). Lead fixed dev_grid ALPHA write (walls were see-through). USER to playtest v7 now. Pending cleanup: other map scenes, mode files (MapSetup branch), MapCatalog modes for wasteland, art modules, unreferenced assets, tools/review probes, CI.
- USER 'lance': game launched windowed for the user (bg biv2uzvcc) + ONE sonnet cleanup agent (modes/MapSetup, other maps, art layer, unreferenced assets incl. incoming, licences, gameplay_probe/CI). Waiting user playtest feedback on v7.
- USER playtest v7: too closed, bump into walls, no room for fast movement; keep physics; wants fewer closed spaces, bigger doors/spaces; purple boundary wall ugly. -> ONE Opus designer v8 (doors >= 2.4/3.5 m, rooms >= 6 m, corridors >= 4 m, stairs >= 3 m open, >= 70 % outdoor, lanes 5 m min with 12 m straight runs, cover 5-10 m apart not near walls, neutral low boundaries; balance kept). Cleanup agent still running.
- 2026-09-26 USER: map too complicated -> tiny Shipment-like map; user will make maps + regenerate models himself; 'clean absolument tout, repartir sur de bonnes bases'. Stopped v8 designer.
  Decisions: KEEP model import tools only (tools/ai3d/bridge_autoexport.py + tools/blender/ai_import_painted.py and deps, licence_check); DELETE docs, backlog (tasks/, tools/tasks, sprint workflow), reports, research, .orchestrator refs; DELETE FP v2 WIP (fp rig, weapons v2, repaint, choreo, assets/models/fp, weapons/v2, textures/weapons) and keep current Ravage + hands; Shipment = plain editable Godot scene (containers), replaces Wasteland (+ its JSON plan tools). Write short README; update CLAUDE.md to the new structure.
  Order: wait cleanup agent acc0a9dba8c2ebf94 -> one sonnet reset agent (deletions + Shipment scene + bots/spawns on static scene + tests + CI) -> lead review, README/CLAUDE.md, commit+push.
- e8e4cf2 pushed (intermediate cleanup: 1651 tests green). ONE sonnet reset agent running: Shipment static editable scene replacing Wasteland, delete FP v2, tools except import/CI, docs/tasks/reports/refs, README; lead will update CLAUDE.md, review screenshots, commit+push.
