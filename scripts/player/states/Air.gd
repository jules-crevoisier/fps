...
func _land() -> void:
	...
	if player.land_roll_buffered():
		...
		transition_to("Roll")
		return
	# Atterrir crouch maintenu + assez vite => slide-hop, sauf saut verrouillé
	# (statut de mouvement, PlayerController.is_jump_locked) -- un atterrissage
	# entravé ne doit pas enchaîner sur une glissade.
